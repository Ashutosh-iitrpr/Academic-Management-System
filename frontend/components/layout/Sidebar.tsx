...
const navigationItems: NavigationItem[] = [
  // Note: Dashboard is handled separately due to role-based routing
  // Admin items
  {
    ...
    >
      ...
      <List sx={{ flex: 1, px: 1, py: 2 }}>
        ...
        <ListItem disablePadding sx={{ mb: 0.5 }}>
          <ListItemButton
            onClick={() => handleNavigate(getDashboardPath(user?.role))}
            sx={{
              borderRadius: 1,
              color: pathname?.includes('/dashboard') ? '#4caf50' : '#b0b0b0',
              backgroundColor: pathname?.includes('/dashboard') ? 'rgba(76, 175, 80, 0.1)' : 'transparent',
              '&:hover': {
